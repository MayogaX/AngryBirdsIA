...
import me.mayogax.ab.planner.abTrajectory;
import me.mayogax.ab.utils.GameImageRecorder;
import me.mayogax.ab.vision.ShowSeg;
			...
			if (args.length == 1 && command.equalsIgnoreCase("-na"))
			{
				...
				if(command.equalsIgnoreCase("-cshoot"))
				{
					...
					if(command.equalsIgnoreCase("-pshoot"))
					{
						...
						{
							ClientMXAgent na = new ClientMXAgent();
							na.run();
						} 
						else 
							if (args.length == 2 && command.equalsIgnoreCase("-nasc"))
							{
								ClientMXAgent na = new ClientMXAgent(args[1]);
								na.run();
							}
							else
								if(args.length == 3 && command.equalsIgnoreCase("-nasc"))
								{
									...
									ClientMXAgent na = new ClientMXAgent(args[1],id);
									na.run();
								}
